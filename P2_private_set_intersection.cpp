...
int main(int argc, char *argv[])
{
    ...
    // =====================================================================
    // BUG FIX: Send max_bin_size to P1 so both parties agree on the OPRF
    // senderSize parameter. Without this, P1 used senderSize=1 while P2
    // used senderSize=bin_sz, causing the OPRF to fail due to mismatched
    // matrix dimensions (width, height, OT parameters).
    // =====================================================================
    char *bin_info = reinterpret_cast<char *>(malloc(sizeof(u64)));
    memcpy(bin_info, &max_bin_size, sizeof(u64));
    sendnetworkmessage(socket, bin_info, sizeof(u64));
    free(bin_info);

    // BUG FIX: Pad ALL bins to max_bin_size with dummy items so that
    // every bin has exactly the same number of items for the OPRF.
    // This ensures the senderSize parameter is consistent across all bins.
    for (u64 i = 0; i < b_bins; i++)
    {
        while (simple.bin_size(i) < max_bin_size)
        {
            simple.table[i].push_back(dummy_item);
        }
    }
    ...
    // Store original (unpadded) bin sizes for T4 serialization
    // Note: after padding above, all bins have max_bin_size items,
    // but we still send all of them since Q needs to try all.

    auto oprf_start = std::chrono::high_resolution_clock::now();

    for (u64 i = 0; i < b_bins; i++)
    {
        // BUG FIX: Use max_bin_size for all bins (now all bins are padded)
        u64 bin_sz = max_bin_size;
    ...
    for (u64 i = 0; i < b_bins; i++)
    {
        u64 bin_sz = max_bin_size;  // All bins are now padded to max_bin_size
