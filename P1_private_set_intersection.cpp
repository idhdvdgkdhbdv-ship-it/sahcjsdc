...
int main(int argc, char *argv[])
{
    ...
    // =====================================================================
    // BUG FIX: Receive max_bin_size from P2 so that both parties agree on
    // the OPRF senderSize parameter. Without this, P1 used senderSize=1
    // while P2 used senderSize=bin_sz, causing the OPRF matrix dimensions
    // (width, height) to differ and the protocol to fail.
    // =====================================================================
    // First, receive max_bin_size from P2 (P2 sends it after simple hashing)
    char *bin_info = receivenetworkmessage(socket, &length);
    assert(length == sizeof(u64));
    u64 max_bin_size = 0;
    memcpy(&max_bin_size, bin_info, sizeof(u64));
    free(bin_info);
    ...
    for (u64 i = 0; i < b_bins; i++)
    {
        ...
        // BUG FIX: Use max_bin_size as senderSize so it matches P2's parameter.
        // Previously this was hardcoded to 1, causing OPRF parameter mismatch.
